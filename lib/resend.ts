...
export async function sendAlertEmail(params: AlertEmailParams): Promise<void> {
  ...
  await resend.emails.send({
    ...
    to,
    subject: `Deal Alert: ${escapeHtml(cardTitle)} — +${roiPct.toFixed(1)}% below market`,
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;">
        <h2 style="color:#6366f1;">CardEdge Deal Alert</h2>
        <p><strong>${escapeHtml(cardTitle)}</strong></p>
        ...
        <table>
          ...
          <tr><td>Watchlist:</td><td>${escapeHtml(watchlistName)}</td></tr>
        </table>
        <a href="${safeUrl(listingUrl)}" style="display:inline-block;margin-top:16px;padding:12px 24px;background:#6366f1;color:#fff;border-radius:6px;text-decoration:none;">View on eBay</a>
